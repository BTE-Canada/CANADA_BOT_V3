...
export const TrialReview: Command = {
    ...
    run: async (client: MyClient, interaction: CommandInteraction) => {
        ...
        if (!DiscordHelpers.hasRole(interaction.member as GuildMember, client.opt.roles.staff)) {
        ...
        const submissionId = (options.get("submissionid", true).value as number).toString();
        ...
        const submissionMsg = await DiscordHelpers.getMessage(client, client.opt.channels.trial_submit, submissionId);
        ...
        if (accept) {
            ...
            DiscordHelpers.getTextChannel(client, client.opt.channels.hi_reviewers)
